...
class ExtendedShimmer @JvmOverloads constructor(
    ...
    init {
        ...
        attrs?.let {
            val typedArray = mContext.obtainStyledAttributes(
                it, R.styleable.CTPLShimmer, 0, 0
            )

            try {
                mLayoutReference = typedArray.getResourceId(
                    R.styleable.CTPLShimmer_shimmer_layout, R.layout.sample_shimmer_row
                )

                shimmerVisibility =
                    typedArray.getInteger(R.styleable.CTPLShimmer_shimmer_visibility, 0)

                mChildCount = typedArray.getInteger(R.styleable.CTPLShimmer_shimmer_child_count, 10)

                mLineAlpha = typedArray.getFloat(R.styleable.CTPLShimmer_shimmer_line_alpha, 0.5f)

                mGridSpan = typedArray.getInteger(R.styleable.CTPLShimmer_shimmer_grid_span, 1)

                mShimmerLayoutManager =
                    typedArray.getInteger(R.styleable.CTPLShimmer_shimmer_layout_manager_type, 0)
    ...
    private fun setUpRecycle() {
        ...
        var adapterRecycler = ExtendedCtplShimmerAdapter(mLayoutReference)
